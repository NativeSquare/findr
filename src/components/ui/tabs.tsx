...
function TabsList({
  ...
  return (
    <TabsPrimitive.List
      className={cn(
        "flex h-auto flex-row items-center justify-center rounded-lg p-[3px]",
  ...
  return (
    <TextClassContext.Provider
      value={cn(
        ...
        value === props.value && "text-primary dark:text-primary"
      )}
    >
      <TabsPrimitive.Trigger
        className={cn(
          "flex flex-1 h-[calc(100%-1px)] flex-row items-center justify-center gap-1.5 border-b-2 border-border px-2 py-3 shadow-none shadow-black/5",
          Platform.select({
